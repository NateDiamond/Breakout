...
STATE_ACTIVE    = 4
#: state when the game is over (either in a win or a loss)
STATE_COMPLETE = 5
...
#: the color sequence of the bricks
BRICK_COLORS = [colormodel.RED, colormodel.ORANGE,colormodel.YELLOW,
                colormodel.GREEN, colormodel.CYAN]
#: the amount the paddle moves each time
PADDLE_SPEED = 5
#: direction when the paddle is not moving
PADDLE_STILL = 0
#: direction when the paddle is moving right
PADDLE_RIGHT = 1
#: direction when the paddle is moving left
PADDLE_LEFT = 2

#: dimensions of sound Icon
SOUND_DIM = 25

#: number of paddle hits between 'kicks'
KICK_INTERVAL = 7
#: factor by which each kick increases the speed
KICK_FACTOR = 1.5

#: amount by which alpha should be incremented in starting animation
A_INC = .008

#: how much time passes in the game for one call to update.
GAME_TIME = .8